...
# The build is always Orchestrion-woven, regardless of which provider is chosen
# at runtime via --tracing.provider.
...
# Fail the build if the OpenTelemetry SDK was dropped, which would leave
# --tracing.provider=otlp silently unable to export anything.
RUN go tool nm /go-ethereum/build/bin/geth | grep -q 'go.opentelemetry.io/otel/sdk' \
    || (echo 'ERROR: geth binary does not carry the OpenTelemetry SDK (OTLP tracing unavailable)' && exit 1)
